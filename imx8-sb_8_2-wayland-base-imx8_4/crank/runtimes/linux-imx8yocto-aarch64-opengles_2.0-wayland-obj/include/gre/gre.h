...
#define GR_BUILD_VERSION 				"¯\\_(:/)_/¯"
#endif
#if !defined(GR_BUILD_REPO)
#define GR_BUILD_REPO
#endif
...
#define GR_APP_LOAD_BINARY				0x2			///< The "bundle" points to the key in the application shared data.													
...
enum {
	...
	GR_LOG_ACTION2,			//Motion/mtevent/redraw action execution
 ...
 * The force variant does not check the log level and logs absolutely.
...
DLLExport void gre_app_log_force(gr_application_t *app, int level, const char *format, ...);
...
DLLExport void gre_app_vlog_force(gr_application_t *app, int level, const char *format, va_list arglist);

/**
 * Registers a handler that will be called when a log message is generated by the engine.  Only one handler
 * can be registered at a time.  
 * @param app The applucation handle, may be NULL to start monitoring early-initialization
